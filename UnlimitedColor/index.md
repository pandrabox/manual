...
### 設定画面が開けない
- UnlimitedColorの設定画面はアバター以下に入っている時のみ使用できます。また、アバターにはVRCAvatarDescriptorが設定されている必要があります

### メニューがでない
- UnlimitedColorプレハブがアバター以下に入っているかどうかご確認下さい
- アバターに使っているシェーダーがlilToonであることをご確認下さい
- Explicit=ONの場合、メニューに有効なRendererを登録していることをご確認下さい（lilToonのRendererが含まれていないと、表示されません）
- Explicit=OFFの場合、過剰に対象外設定していないかご確認下さい
